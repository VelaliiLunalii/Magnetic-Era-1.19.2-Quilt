...
import io.github.velaliilunalii.magnetic_era.block.custom.capacitor.CalibratedCopperCapacitorBlock;
...
public class CalibratedCopperCapacitorBlockEntity extends BlockEntity implements BlockEntityTicker<CalibratedCopperCapacitorBlockEntity> {
	private int charge;
	private int checkCooldown = 0;
	private int maxCharge = 1;

	public CalibratedCopperCapacitorBlockEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.CALIBRATED_COPPER_CAPACITOR_BLOCK_ENTITY, pos, state);
		if (state.getBlock() instanceof CalibratedCopperCapacitorBlock capacitor) {
			this.maxCharge = capacitor.getMaxCharge();
		}
	}

	@Override
	public void tick(World world, BlockPos pos, BlockState state, CalibratedCopperCapacitorBlockEntity blockEntity) {
