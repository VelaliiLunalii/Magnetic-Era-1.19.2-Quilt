...
public class IronCapacitorBlockEntity extends BlockEntity implements BlockEntityTicker<IronCapacitorBlockEntity> {
	private int charge;
	private int checkCooldown = 0;
	private int maxCharge = 1;

	public IronCapacitorBlockEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.IRON_CAPACITOR_BLOCK_ENTITY, pos, state);
		if (state.getBlock() instanceof IronCapacitorBlock capacitor) {
			this.maxCharge = capacitor.getMaxCharge();
		}
	}

	@Override
	public void tick(World world, BlockPos pos, BlockState state, IronCapacitorBlockEntity blockEntity) {
