...
import io.github.velaliilunalii.magnetic_era.block.custom.capacitor.CalibratedNetheriteCapacitorBlock;
...
public class CalibratedNetheriteCapacitorBlockEntity extends BlockEntity implements BlockEntityTicker<CalibratedNetheriteCapacitorBlockEntity> {
	private int charge;
	private int checkCooldown = 0;
	private int maxCharge = 1;

	public CalibratedNetheriteCapacitorBlockEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.CALIBRATED_NETHERITE_CAPACITOR_BLOCK_ENTITY, pos, state);
		if (state.getBlock() instanceof CalibratedNetheriteCapacitorBlock capacitor) {
			this.maxCharge = capacitor.getMaxCharge();
		}
	}

	@Override
	public void tick(World world, BlockPos pos, BlockState state, CalibratedNetheriteCapacitorBlockEntity blockEntity) {
