package io.github.velaliilunalii.vegetarianism.entity.custom;
...
public class MetalDetectorBobberEntity extends FishingBobberEntity {
	public MetalDetectorBobberEntity(EntityType<? extends FishingBobberEntity> type, World world, int luckOfTheSeaLevel, int lureLevel) {
		super(type, world, luckOfTheSeaLevel, lureLevel);
	}

	public MetalDetectorBobberEntity(EntityType<? extends FishingBobberEntity> entityType, World world) {
		super(entityType, world);
	}

	public MetalDetectorBobberEntity(PlayerEntity thrower, World world, int luckOfTheSeaLevel, int lureLevel) {
	...
	private boolean removeIfInvalid(PlayerEntity player) {
		...
		boolean bl = itemStack.isOf(ModItems.METAL_DETECTOR_ROD);
		boolean bl2 = itemStack2.isOf(ModItems.METAL_DETECTOR_ROD);
	...
	public int use(ItemStack usedItem) {
		...
		if (!this.world.isClient && playerEntity != null && !removeIfInvalid(playerEntity)) {
			int i = 0;
...
//				LootContext.Builder builder = (new LootContext.Builder((ServerWorld)this.world)).parameter(LootContextParameters.ORIGIN, this.getPos()).parameter(LootContextParameters.TOOL, usedItem).parameter(LootContextParameters.THIS_ENTITY, this).random(this.random).luck((float)accessor.getLuckOfTheSeaLevel() + playerEntity.getLuck());
//				LootTable lootTable = this.world.getServer().getLootManager().getTable(ModLootTables.METAL_DETECTOR_FISHING);
//				List<ItemStack> list = lootTable.generateLoot(builder.build(LootContextTypes.FISHING));
				...
				if (roll < fishWeight) {
					tableToUse = new Identifier(Vegetarianism.MODID, "gameplay/metal_detector_fishing");
