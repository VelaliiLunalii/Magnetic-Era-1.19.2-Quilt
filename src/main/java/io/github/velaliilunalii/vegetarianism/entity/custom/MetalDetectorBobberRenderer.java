...
public class MetalDetectorBobberRenderer extends EntityRenderer<MetalDetectorBobberEntity> {
	private static final Identifier TEXTURE = new Identifier(Vegetarianism.MODID, "textures/entity/metal_detector_bobber.png");
	private static final RenderLayer LAYER;
	private static final double BOBBING_VIEW_SCALE = (double)960.0F;

	public MetalDetectorBobberRenderer(EntityRendererFactory.Context ctx) {
		super(ctx);
	}

	public void render(MetalDetectorBobberEntity fishingBobberEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
		...
		if (playerEntity != null) {
			...
			if (!itemStack.isOf(ModItems.METAL_DETECTOR_ROD)) {
				j = -j;
			}
	...
	public Identifier getTexture(MetalDetectorBobberEntity fishingBobberEntity) {
		return TEXTURE;
	}
